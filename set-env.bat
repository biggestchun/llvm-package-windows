...
:clang
set TOOLCHAIN=clang
:: Default to NMake if generator not specified
...
if /i "%GENERATOR:~0,13%" == "Visual Studio" (
    if "%CMAKE_USE_ARCH_OPTIONS%" == "" (
        set CMAKE_GENERATOR=%GENERATOR%%CMAKE_GENERATOR_SUFFIX%
    ) else (
        set CMAKE_OPTIONS=%CMAKE_OPTIONS% %CMAKE_ARCH_OPTIONS%
    )
) else (
    :: For Ninja or NMake with Clang
    if "%TOOLCHAIN%" == "clang" (
...
:: Set build flags based on generator
if /i "%GENERATOR:~0,13%" == "Visual Studio" (
...
:: Add host platform for Visual Studio generators
if /i "%GENERATOR:~0,13%" == "Visual Studio" (
    set LLVM_CMAKE_CONFIGURE_FLAGS=%LLVM_CMAKE_CONFIGURE_FLAGS% -Thost=x64
)

:: Add CRT flags and extra configure flags
set LLVM_CMAKE_CONFIGURE_FLAGS=%LLVM_CMAKE_CONFIGURE_FLAGS% %LLVM_CMAKE_CRT_FLAGS% %LLVM_CMAKE_CONFIGURE_EXTRA_FLAGS%
...
:: Add host platform for Visual Studio generators
if /i "%GENERATOR:~0,13%" == "Visual Studio" (
    set CLANG_CMAKE_CONFIGURE_FLAGS=%CLANG_CMAKE_CONFIGURE_FLAGS% -Thost=x64
)

:: Add CRT flags and extra configure flags
set CLANG_CMAKE_CONFIGURE_FLAGS=%CLANG_CMAKE_CONFIGURE_FLAGS% %LLVM_CMAKE_CRT_FLAGS% %CLANG_CMAKE_CONFIGURE_EXTRA_FLAGS%
